...
-- Mevcut veritabanlarında kullanmak için tasarlanmıştır
...
-- 3. offer_items tablosunda da aynı değişiklikleri yap
ALTER TABLE offer_items 
ADD COLUMN IF NOT EXISTS product_name_tr VARCHAR(200),
ADD COLUMN IF NOT EXISTS product_name_en VARCHAR(200);

-- 4. Mevcut product_name verilerini product_name_tr'ye kopyala
UPDATE offer_items 
SET product_name_tr = product_name 
WHERE product_name_tr IS NULL AND product_name IS NOT NULL;

-- ====================================
-- İSTEĞE BAĞLI: ESKİ KOLONLARI KALDIR
-- ====================================
-- Sadece eski kolonların artık kullanılmadığından eminseniz açın:

-- -- Eski name kolonunu kaldır (dikkatli olun, önce backup alın!)
-- ALTER TABLE pricelist_items DROP COLUMN IF EXISTS name;

-- -- Eski product_name kolonunu kaldır (dikkatli olun!)
-- ALTER TABLE offer_items DROP COLUMN IF EXISTS product_name;

-- ====================================
-- NOT: Bu script çalıştırıldıktan sonra
-- frontend ve backend kodları dual-language
-- sistemini destekler hale gelmiştir
-- ====================================
